function [val, diagnostics, fSOS, fSONC, hSOS, hSONC, lambdaOpt] = ...
    boundedSOSplusSONC(f, gList, x, k, d, verb)
...
%% Compute Newton polytope (shared basis for both SOS and SONC)
[monSONC, expSONC] = newtonPolytope(P, x);
numMonSONC = length(monSONC);

%% SOS block — same basis as SONC
monSOS = monSONC;
nSOS   = length(monSOS);
Q      = sdpvar(nSOS);
...
%% SONC block
% Identify non-square monomials (odd exponents → inner terms)
...
if diagnostics.problem == 0
    ...
    if nargout >= 5
        ...
        for i = 1:numMonSONC
            hSONC(i) = dec((numMonSONC*(i-1))+1 : numMonSONC*i)' * monSONC;
        end
    end
